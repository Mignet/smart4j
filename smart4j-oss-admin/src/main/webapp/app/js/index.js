;(function ($win,$) {
        ...
        $win.v5ent= {routerTree:[]};
        v5ent.router={
        breadcrumbChangeEvent:function () {
            var nodes = v5ent.routerTree;
//            console.log(nodes);
        ...
        v5ent.addRouterTree = function (title,url) {
            ...
            if(is_parent){
                v5ent.routerTree = [{title:title,url:url}];
            }
            else{
                ...
                if(!hasnode || nodes.length<=0){
                    v5ent.routerTree.push({title:title,url:url});
                }

            }
            v5ent.router.breadcrumbChangeEvent();
...
$(function() {
    ...
    var Index = (function() {
        ...
        me.switchPage = function (pageName,pageUrl) {
            var $method = "GET";
            v5ent.addRouterTree(pageName,pageUrl);
            changeMenu(pageName);
            $.ajax({
                type :$method,
                url :pageUrl,
        ...
        me.handleSubMenuClick = function() {
            ...
            $('#page-sidebar-menu li a').click(function(e) {
                ...
                if (url !== null && url !== 'javascript:;') {
                	//设置引导显示menu>menu2
                    v5ent.addRouterTree(this.text,url,true);
